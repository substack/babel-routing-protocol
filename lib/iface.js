...
function Iface (id, addr, router) {
  if (!(this instanceof Iface)) return new Iface(id, addr, router)
  var self = this
  self.router = router
  self.id = id
  Duplex.call(self)
  ...
  self.intervals.update = setInterval(function () {
    self.push(encode(Object.keys(self.router.D).map(function (raddr) {
      return {
        type: 'update',
        addrEnc: 1,
        ...
        flags: 0,
        omitted: 0,
        metric: self.router.D[raddr],
        cseq: 0,
        seq: self.router.seqno ++
      }
    })))
  }, self.updateMsec)
}
